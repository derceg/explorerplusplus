...
void BuildV2LoadSaveReferenceTree(BookmarkTree *bookmarkTree)
{
	...
	auto subFolder = std::make_unique<BookmarkItem>(
		L"F4382A95-DCC2-41D3-8675-98243D9956A1", L"Sub Folder 1", std::nullopt);
	...
	auto bookmark = std::make_unique<BookmarkItem>(
		L"CC620AF8-6761-4A1C-A7CD-CD23F5054FBD", L"Second drive", L"D:\\");
	...
	bookmark = std::make_unique<BookmarkItem>(
		L"08232430-DAF7-45DA-91FC-A350459C08CA", L"Public desktop", L"C:\\Users\\Public\\Desktop");
	bookmarkTree->AddBookmarkItem(
		bookmarkTree->GetBookmarksToolbarFolder(), std::move(bookmark), 1);

	folder = std::make_unique<BookmarkItem>(
		L"DB3C84AB-5DE8-4820-B770-45F6C3C275EE", L"Other Folder 1", std::nullopt);
	rawFolder = bookmarkTree->AddBookmarkItem(
		bookmarkTree->GetOtherBookmarksFolder(), std::move(folder), 0);

	bookmark = std::make_unique<BookmarkItem>(
		L"B0AB36D9-E6B0-4D39-B24B-72BEB13F1D78", L"Sys32", L"C:\\Windows\\System32");

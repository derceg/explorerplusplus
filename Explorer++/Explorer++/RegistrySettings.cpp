...
LONG Explorerplusplus::SaveSettings(void)
{
	...
	if(ReturnValue == ERROR_SUCCESS)
	{
		...
		NRegistrySettings::SaveDwordToRegistry(hSettingsKey,_T("ShowFolderSizes"),m_config->showFolderSizes);
		...
		NRegistrySettings::SaveDwordToRegistry(hSettingsKey,_T("ShowPrivilegeLevelInTitleBar"),m_config->showPrivilegeLevelInTitleBar);
		...
		NRegistrySettings::SaveDwordToRegistry(hSettingsKey,_T("ForceSize"),m_config->forceSize);
		NRegistrySettings::SaveDwordToRegistry(hSettingsKey,_T("SizeDisplayFormat"),m_config->sizeDisplayFormat);
...
LONG Explorerplusplus::LoadSettings()
{
	...
	if(ReturnValue == ERROR_SUCCESS)
	{
		...
		NRegistrySettings::ReadDwordFromRegistry(hSettingsKey,_T("ShowFolderSizes"),(LPDWORD)&m_config->showFolderSizes);
		...
		NRegistrySettings::ReadDwordFromRegistry(hSettingsKey,_T("ShowPrivilegeLevelInTitleBar"),(LPDWORD)&m_config->showPrivilegeLevelInTitleBar);
		...
		NRegistrySettings::ReadDwordFromRegistry(hSettingsKey,_T("ForceSize"),(LPDWORD)&m_config->forceSize);
		NRegistrySettings::ReadDwordFromRegistry(hSettingsKey,_T("SizeDisplayFormat"),(LPDWORD)&m_config->sizeDisplayFormat);
	...
	if(ReturnValue == ERROR_SUCCESS)
	{
		...
		for(i = 0;i < iNumTabs;i++)
		{
			...
			if(ReturnValue == ERROR_SUCCESS)
			{
				...
				pidlDirectory = tab.shellBrowser->QueryCurrentDirectoryIdl();
				...
				NRegistrySettings::SaveDwordToRegistry(hTabKey,_T("SortAscending"), tab.shellBrowser->GetSortAscending());
				NRegistrySettings::SaveDwordToRegistry(hTabKey,_T("ShowInGroups"), tab.shellBrowser->IsGroupViewEnabled());
				NRegistrySettings::SaveDwordToRegistry(hTabKey,_T("ApplyFilter"), tab.shellBrowser->GetFilterStatus());
				NRegistrySettings::SaveDwordToRegistry(hTabKey,_T("FilterCaseSensitive"), tab.shellBrowser->GetFilterCaseSensitive());
				NRegistrySettings::SaveDwordToRegistry(hTabKey,_T("ShowHidden"), tab.shellBrowser->GetShowHidden());
				NRegistrySettings::SaveDwordToRegistry(hTabKey,_T("AutoArrange"), tab.shellBrowser->GetAutoArrange());
				NRegistrySettings::SaveDwordToRegistry(hTabKey,_T("ShowGridlines"), tab.shellBrowser->QueryGridlinesActive());
				...
				if(ReturnValue == ERROR_SUCCESS)
				{
					...
					tab.shellBrowser->ExportAllColumns(&cie);
				...
				if(tab.bUseCustomName)
					NRegistrySettings::SaveStringToRegistry(hTabKey,_T("CustomName"),tab.szName);
				else
...
int Explorerplusplus::LoadTabSettingsFromRegistry(void)
{
	...
	if(ReturnValue == ERROR_SUCCESS)
	{
		...
		while(ReturnValue == ERROR_SUCCESS)
		{
			...
			Settings.bShowFolderSizes = m_config->showFolderSizes;

...
LRESULT CALLBACK TabDropHandler::TabCtrlProcStub(
...
HRESULT __stdcall TabDropHandler::DragEnter(
	IDataObject *pDataObject, DWORD grfKeyState, POINTL pt, DWORD *pdwEffect)
{
	...
	*pdwEffect = DetermineCurrentDragEffect(iTab, grfKeyState, *pdwEffect);
	...
	m_pDropTargetHelper->DragEnter(
		m_hTabCtrl, pDataObject, reinterpret_cast<POINT *>(&pt), *pdwEffect);
	...
	if (hr == S_OK)
	{
		...
		if (pdf != nullptr)
		{
			...
			if (nDroppedFiles >= 1)
			{
				...
				DragQueryFile(
					reinterpret_cast<HDROP>(pdf), 0, szFileName, SIZEOF_ARRAY(szFileName));
...
HRESULT __stdcall TabDropHandler::DragOver(DWORD grfKeyState, POINTL pt, DWORD *pdwEffect)
{
	...
	focus to this tab. */
	if (iTab != -1 && iTab != m_tabContainer->GetSelectedTabIndex() && iTab != m_TabHoverIndex)
	{
		SetTimer(m_hTabCtrl, TIMER_ID, TIMEOUT_VALUE, nullptr);
	}
	else if (iTab == -1 || iTab != m_TabHoverIndex)
	{
...
HRESULT __stdcall TabDropHandler::Drop(
	IDataObject *pDataObject, DWORD grfKeyState, POINTL pt, DWORD *pdwEffect)
{
	KillTimer(m_hTabCtrl, TIMER_ID);

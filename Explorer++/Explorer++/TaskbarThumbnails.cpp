...
LRESULT CALLBACK Explorerplusplus::TabProxyWndProc(HWND hwnd,UINT Msg,WPARAM wParam,LPARAM lParam,int iTabId)
{
	...
	case WM_ACTIVATE:
		...
		OnSelectTab(m_Tabs.at(iTabId), FALSE);
		return 0;
		break;

	case WM_SETFOCUS:
		SetFocus(m_Tabs.at(iTabId).listView);
		break;
	...
	case WM_SYSCOMMAND:
		...
		default:
			SendMessage(m_Tabs.at(iTabId).listView,WM_SYSCOMMAND,wParam,lParam);
			break;
		}
		break;
	...
	case WM_CLOSE:
		{
			...
			else
			{
				for(i = 0;i < nTabs;i++)
				{
					...
					{
						TabInfo_t tab = m_Tabs.at(iTabId);
						CloseTab(tab);
						break;
					}
				}
...
HBITMAP Explorerplusplus::CaptureTabScreenshot(int iTabId)
{
	...
	HWND hTab = m_Tabs.at(iTabId).listView;

...
void TabContainer::Initialize(HWND parent)
{
	...
	m_connections.push_back(tabNavigationStartedSignal.AddObserver(
		boost::bind(&TabContainer::OnNavigationStarted, this, _1, _2, _3)));
	m_connections.push_back(
		tabSelectedSignal.AddObserver(boost::bind(&TabContainer::OnTabSelected, this, _1)));
	m_connections.push_back(
		tabRemovedSignal.AddObserver(boost::bind(&TabContainer::OnTabRemoved, this, _1)));

	m_connections.push_back(m_config->alwaysShowTabBar.addObserver(
		boost::bind(&TabContainer::OnAlwaysShowTabBarUpdated, this, _1)));
	m_connections.push_back(m_config->forceSameTabWidth.addObserver(
		boost::bind(&TabContainer::OnForceSameTabWidthUpdated, this, _1)));
}
...
void TabContainer::OnNavigationStarted(const Tab &tab, PCIDLIST_ABSOLUTE pidl, bool addHistoryEntry)
{
	...
	const TabSettings &tabSettings, bool addHistoryEntry, int *newTabId)
{
	...
	tab.GetShellBrowser()->AddNavigationStartedObserver(
		[this, &tab](PCIDLIST_ABSOLUTE pidl, bool addHistoryEntry) {
			tabNavigationStartedSignal.m_signal(tab, pidl, addHistoryEntry);
		});

	// Capturing the tab by reference here is safe, since the tab object is
	// guaranteed to exist whenever this method is called.
	tab.GetShellBrowser()->AddNavigationCompletedObserver([this, &tab]() {

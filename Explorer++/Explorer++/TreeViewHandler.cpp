...
void Explorerplusplus::CreateFolderControls(void)
{
	...
	m_pMyTreeView = new CMyTreeView(m_hTreeView, m_hContainer, m_pDirMon);
	...
	case WM_MBUTTONUP:
		{
			...
			if(tvhi.flags != LVHT_NOWHERE && tvhi.hItem != NULL)
			{
				...
				{
					auto pidl = m_pMyTreeView->BuildPath(tvhi.hItem);
...
void Explorerplusplus::OnTreeViewRightClick(WPARAM wParam,LPARAM lParam)
{
	...
	if(SUCCEEDED(hr))
	{
		...
		if(hParent != NULL)
		{
			auto pidlParent = m_pMyTreeView->BuildPath(hParent);
...
{
	auto pidlDirectory = m_pMyTreeView->BuildPath(g_NewSelectionItem);
...
int Explorerplusplus::OnTreeViewBeginLabelEdit(LPARAM lParam)
{
	...
	auto pidl = m_pMyTreeView->BuildPath(pdi->item.hItem);
...
void Explorerplusplus::OnTreeViewSetFileAttributes(void) const
{
	...
	auto pidlItem = m_pMyTreeView->BuildPath(hItem);

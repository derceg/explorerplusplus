	...
	BookmarkMenuBuilder::ItemIdMap &menuItemIdMappings)
{
	...
	AddBookmarkItemsToMenu(
		menu.get(), GetMenuItemCount(menu.get()), menuImages, menuItemIdMappings);
	AddOtherBookmarksToMenu(
		menu.get(), GetMenuItemCount(menu.get()), menuImages, menuItemIdMappings);

	return menu;
}

void BookmarksMainMenu::AddBookmarkItemsToMenu(HMENU menu, int position,
	std::vector<wil::unique_hbitmap> &menuImages,
	BookmarkMenuBuilder::ItemIdMap &menuItemIdMappings)
{
	...
	m_menuBuilder.BuildMenu(m_expp->GetMainWindow(), menu, bookmarksMenuFolder, m_menuIdRange,
		position, menuItemIdMappings, menuImages);
}

void BookmarksMainMenu::AddOtherBookmarksToMenu(HMENU menu, int position,
	std::vector<wil::unique_hbitmap> &menuImages,
	BookmarkMenuBuilder::ItemIdMap &menuItemIdMappings)
{
	...
	m_menuBuilder.BuildMenu(m_expp->GetMainWindow(), subMenu, otherBookmarksFolder, m_menuIdRange,
		0, menuItemIdMappings, menuImages);
